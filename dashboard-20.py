...
with tabs[1]:
    ...
    **📝 Notes:** {detail["Remarks"] if pd.notnull(detail["Remarks"]) else '-'}
    ...
    with tabs[0]:
        ...
        st.subheader("📋 Daftar Proyek Prioritas (Scroll Otomatis)")

        # Styling warna kolom prioritas
        priority_style = JsCode("""
        function(params) {
            if (params.value === 'Tinggi') {
                return {color: 'white', backgroundColor: '#ff4d4d'};
            } else if (params.value === 'Sedang') {
                return {color: 'black', backgroundColor: '#ffd966'};
            } else if (params.value === 'Rendah') {
        ...
        # Konfigurasi AgGrid
        gb = GridOptionsBuilder.from_dataframe(df_scroll)
        gb.configure_default_column(resizable=True, filter=True, sortable=True)
        gb.configure_column("Priority Level", cellStyle=priority_style)
        ...
        # Inject scroll otomatis
        scroll_script = """
        <script>
        const scrollContainer = window.parent.document.querySelector('iframe').parentNode;
        let scrollTop = 0;
        setInterval(() => {
            scrollTop += 1;
